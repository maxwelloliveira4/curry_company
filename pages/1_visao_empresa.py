...
def clean_code(df1):
    ...
    return df1 # aqui termina a limpeza 

def order_metric(df1):  # aqui começa a definição dos gráficos
    cols = ['ID', 'Order_Date']
...
# (essa parte da imagem nao fiz) image = Image.open('1.png') # Image é uma função da biblioteca PIL, open uma função (livro) dela, que abre fotos do meu computador.
...
st.sidebar.markdown("""---""") 
which_city = st.sidebar.multiselect(
    'Qual cidade',
...
# Filtro de cidade:
linhas_selecionadas = df1['City'].isin(which_city) # which_city estão em >>> filtro which_city.
df1 = df1.loc[linhas_selecionadas, :]
